...
export async function installGitPayload(repo: string, sha: string, runCommand: CommandRunner, paths = resolveInstallStorePaths()): Promise<{ payloadPath: string; reused: boolean; version: string }> {
  ...
  try {
    ...
    // @paperclipai/server bundles ui-dist (it lists "ui-dist" in its
    // package.json "files"), but ui is not a package.json dependency of
    // server, so the recursive --filter build above never builds it.
    // scripts/release.sh avoids this by running a repo-wide `pnpm build`
    // before packaging; mirror that narrowly here so the bundling step below
    // has something to copy.
    await runCommand("corepack", ["pnpm", "--filter", "@paperclipai/ui", "build"], { cwd: checkoutPath, env: buildEnv(), maxBuffer: 32 * 1024 * 1024 });
